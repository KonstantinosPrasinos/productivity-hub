...
const getRecentEntries = async (req, res) => {
  if (req.user) {
    ...
    const entries = await Entry.find({ userId: req.user._id, taskId: taskId })
      .sort({ $natural: -1 })
      .limit(7);
...
const setEntryValue = (req, res) => {
  if (req.user) {
    ...
    if (isNaN(value) || !taskId || !entryId)
      return res
        .status(400)
        .json({ message: "Invalid task id, entry id or value." });

    Entry.findOneAndUpdate(
      { userId: req.user._id, _id: entryId },
      { $set: { value: value } },
      ...
      (err, entry) => {
        if (err)
          return res
            .status(400)
            .json({ message: "Failed to set entry value." });
        return res.status(200).json({ entry });
      },
    );
  } else {
...
const addEntry = async (req, res) => {
  if (req.user) {
    ...
    if (!date || isNaN(value) || !taskId)
      return res
        .status(400)
        .json({ message: "Date, value and task id required." });
