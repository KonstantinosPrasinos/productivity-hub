...
const findMostRecentDate = (task, currentDate = null) => {
  ...
  const { functionName, timeToAdd } = getDateAddDetails(
    task.repeatRate.bigTimePeriod,
    task.repeatRate.number
  );
...
const assembleEntryHistory = (entries, task, currentDate = null) => {
  // current date for tests
  let streak = 0;
  ...
  while (entryFound) {
    for (let i = task.repeatRate.startingDate.length - 1; i >= 0; i--) {
      let timeDifference =
        task.repeatRate.startingDate[i] - task.repeatRate.startingDate[0];
  ...
  return {
    streak,
    date: streakStartDate ? new Date(streakStartDate) : streakStartDate,
  };
};
...
const getTasks = async (req, res) => {
  if (req.user) {
    Task.find(
      { userId: req.user._id, forDeletion: false },
      async (err, tasks) => {
        if (err) return res.status(404).json({ message: "Tasks not found." });

        const { tasksWithCurrentEntry, currentEntries } =
          await getTasksWithHistory(tasks, req.user._id);

        if (tasks) {
          return res.status(200).json({
            tasks: tasksWithCurrentEntry,
  ...
  if (req.user) {
    ...
    try {
      await Task.updateMany(
        { userId: req.user._id, _id: taskId },
        { $set: { forDeletion: true } }
      );

      await Entry.updateMany(
        { userId: req.user._id, taskId: taskId },
        { $set: { forDeletion: true } }
      );

      res
        .status(200)
        .json({ message: "Task and it's entries set for deletion." });
    } catch (error) {
